...
def images(G,device,inputs,space,truncation_psi,label,noise_mode,outdir,start=None,stop=None):
    ...
    for idx, i in enumerate(inputs):
        ...
        # i가 NumPy 배열인지 PyTorch 텐서인지 확인
        if isinstance(i, np.ndarray):
            z = torch.from_numpy(i).to(device)
        elif isinstance(i, torch.Tensor):
            z = i.to(device)
        else:
            raise TypeError(f"Unsupported type for points: {type(i)}")
           
        if (space=='z'):
            # 이미 설정된 z 변수 사용 (i를 다시 변환하지 않음)
            if(start is not None and stop is not None):
...
def slerp(t, v0, v1, DOT_THRESHOLD=0.9995):
    ...
    '''
    # v0와 v1이 PyTorch 텐서인지 확인하고, 필요한 경우 NumPy 배열로 변환
